...
4. Generate custom skills via `/skills aif-skill-generator` (pass URLs for Learn Mode when docs are available)
5. Configure MCP in `.qwen/settings.json`
6. Generate `AGENTS.md` in project root (see [AGENTS.md Generation](#agentsmd-generation))
7. Generate architecture document via `/skills aif-architecture` (see [Architecture Generation](#architecture-generation))
...
Install skills, configure MCP, generate `AGENTS.md`, and generate architecture document via `/skills aif-architecture` as in Mode 1.
...
- /skills aif-roadmap — Create a strategic roadmap with milestones (recommended for new projects)
- /skills aif-plan <description> — Plan implementation (creates branch + plan, or quick plan)
- /skills aif-implement — Execute existing plan
...
- /skills aif-docs — Generate project documentation
- /skills aif-rules — Add project-specific rules and conventions
- /skills aif-build-automation — Configure build scripts and automation
- /skills aif-ci — Set up CI/CD pipeline
- /skills aif-dockerize — Containerize the project

Would you like to run any of these now?
```

Present these as `AskUserQuestion` with multi-select options:
1. Generate docs (`/skills aif-docs`)
2. Build automation (`/skills aif-build-automation`)
3. CI/CD (`/skills aif-ci`)
4. Dockerize (`/skills aif-dockerize`)
