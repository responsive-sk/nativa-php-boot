...
- Does `/skills aif-fix` mention the most common error categories from patches? If not → add them
- Does `/skills aif-implement` warn about the pitfalls found in patches? If not → add guards
- Does `/skills aif-plan` include logging/validation requirements for problem areas? If not → enhance
- Does `/skills aif-review` check for the patterns that caused bugs? If not → add checklist items
...
|-------|-------------|---------------------|
| `/skills aif-fix` | Patches → common errors | "Check for X before accessing Y" |
| `/skills aif-implement` | Patches → prevention rules | "When creating models, always validate Z" |
| `/skills aif-plan` | Patches → logging gaps | "Add validation task for nullable fields" |
| `/skills aif-review` | Patches → missed issues | "Check: are all optional relations null-safe?" |
| `/skills aif-commit` | Codebase → conventions | "Use project's commit prefix format" |
...
→ Improvement: Add to /skills aif-fix Step 2:
...
→ Improvement: Add to /skills aif-review checklist:
...
- Do not create new skills (suggest using `/skills aif-skill-generator` instead)
