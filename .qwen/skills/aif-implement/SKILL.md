...
- [ ] Yes, commit now (/skills aif-commit)
...
→ **Invoke `/skills aif-fix`** (without arguments — it will detect FIX_PLAN.md and execute it).
...
- Return to main → `git checkout main && git pull` → `/skills aif-plan full <description>`
...
```
1. .ai-factory/PLAN.md exists? → Use it (from /skills aif-plan fast)
...
**Priority:**
1. `.ai-factory/PLAN.md` - always takes priority (from `/skills aif-plan fast`)
2. Branch-named file - if no .ai-factory/PLAN.md (from `/skills aif-plan full`)
3. `.ai-factory/FIX_PLAN.md` - redirect to `/skills aif-fix` (from `/skills aif-fix` plan mode)
...
If user chooses "Verify first" → suggest invoking `/skills aif-verify`.
If user chooses "Skip to commit" → suggest invoking `/skills aif-commit`.

**Check if documentation needs updating:**

Read the plan file settings. If documentation preference is set to "yes" (from `/skills aif-plan full` questions), run `/skills aif-docs` to update documentation.
...
```
→ Invoke `/skills aif-docs` to analyze changes and update docs.

**Handle plan file after completion:**

- **If `.ai-factory/PLAN.md`** (from `/skills aif-plan fast`):
  ```
