...
final class FormValidationService
{
    ...
    public function validate(array $data, array $schema): bool
    {
        ...
        foreach ($schema as $field) {
            ...
            if (null === $sanitizedValue && $this->hasError($name)) {
                continue;
            }
     ...
     * @param array<string, mixed> $field
     */
    private function sanitizeNumber(mixed $value, array $field): float | int | null
    {
        $name = $field['name'];
        $label = $field['label'] ?? ucfirst($name);
        ...
        $numericValue = 'tel' === $type ? (int) $value : (float) $value;
        ...
        return 'tel' === $type ? (int) $value : (float) $value;
    }
