...
final class TemplateRenderer
{
    ...
    public function render(string $template, array $data = [], ?string $layout = null): string
    {
        ...
        if ($this->currentLayout) {
            ...
            return false !== $layoutResult ? $layoutResult : '';
        }
     ...
     * @param array<string, mixed> $data
     */
    private function renderTemplate(string $template, array $data = []): false | string
    {
        ...
        if (isset($this->templateCache[$cacheKey])) {
            ...
            if (false === $cacheMtime || $templateMtime > $cacheMtime) {
                $this->compileTemplate($templatePath, $cacheFile);
            }
     ...
     */
    private function renderLayout(string $layout): false | string
    {
        ...
        if ('admin' === $layout || str_starts_with($layout, 'admin')) {
            $layoutPath .= '/admin.php';
        } elseif ('frontend' === $layout || str_starts_with($layout, 'frontend')) {
    ...
    private function loadCacheVersion(): string
    {
        ...
        if (file_exists($versionFile)) {
            ...
            return false !== $content ? trim($content) : date('YmdHis');
        }
